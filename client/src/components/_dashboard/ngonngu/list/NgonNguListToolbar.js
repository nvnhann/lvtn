...
// ----------------------------------------------------------------------
export default function NgonNguListToolbar({
  selected,
  filterName,
  onFilterName,
  setLoad,
  ...
  const funcHidden = async () => {
    try {
      await putData(API_BASE_URL + '/api/ngonngu-active', {
      ...
      <DialogConfirm
        ...
        message={
          <Typography color="error" variant="h4" align="center">
            Bạn chắc chắn muốn {!!hidden ? 'ẩn' : 'hiện'} các ngôn ngữ?
          </Typography>
        }
        excFunc={funcHidden}
      />

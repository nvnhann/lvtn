...
// ----------------------------------------------------------------------
export default function NXBListToolbar({
  selected,
  filterName,
  onFilterName,
  setLoad,
  ...
  const funcHidden = async () => {
    try {
      await putData(API_BASE_URL + '/api/nhaxuatban-active', {
      ...
      <DialogConfirm
        ...
        message={
          <Typography color="error" variant="h4" align="center">
            Bạn chắc chắn muốn {!!hidden ? 'ẩn' : 'hiện'} các nhà xuất bản?
          </Typography>
        }
        excFunc={funcHidden}
      />

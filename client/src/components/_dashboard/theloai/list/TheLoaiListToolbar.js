...
// ----------------------------------------------------------------------
export default function TheLoaiListToolbar({
  selected,
  filterName,
  onFilterName,
  setLoad,
  ...
  const funcHidden = async () => {
    try {
      await putData(API_BASE_URL + '/api/theloai-active', {
      ...
      <DialogConfirm
        ...
        message={
          <Typography color="error" variant="h4" align="center">
            Bạn chắc chắn muốn {!!hidden ? 'ẩn' : 'hiện'} các thể loại?
          </Typography>
        }
        excFunc={funcHidden}
      />

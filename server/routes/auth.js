...
module.exports = function (app) {
        ...
        await db.query(
            ...
            async (err, data) => {
                ...
                if (data.length !== 0) {
                    ...
                    await db.query(sql, [value], (err, data) => {
                        if (err) return res.status(500);
                        let _user = {
                            id: data.insertId,
    ...
    app.post('/auth/change-password', async (req, res) => {
        ...
        if (_user[0].credential) {
            ...
            if (!passwordIsValid) return res.status(500).send("Mật khẩu cũ không chính xác");
        }
        await query(db, "UPDATE users SET credential = ? WHERE email = ?", [bcrypt.hashSync(newPass, 8), email])
    ...
    app.post('/auth/reset-password', async (req, res) => {
        ...
        if (decode.email) {
            await query(db, "UPDATE users SET credential = ? WHERE email = ?", [bcrypt.hashSync(newPass, 8), decode.email]);
            return res.status(200).send("Đổi mật khẩu thành công!");
        }

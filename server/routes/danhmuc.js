...
module.exports = function (app) {
    ...
    app.post("/danhmuc/active", async (req, res) => {
        ...
        await query(db, `UPDATE danh_muc SET active = ? where dm_id = ?`, [active, id]);
    ...
    app.get("/danhmuc/:id", async (req, res) => {
        ...
        return res.status(200).send(await query(db, `SELECT * FROM danh_muc where dm_id = ?`, id));
    });
    ...
    app.post("/danhmuc/create", async (req, res) => {
        ...
        const isExits = await query(db, `SELECT * FROM danh_muc where dm_ten = ?`, data.dm_ten);
    ...
    app.delete("/danhmuc/delete", async (req, res) => {
        if (!!req.body.arrID) {
            ...
            await Promise.all(arrID.map(async (e) => await query(db, `DELETE FROM danh_muc where dm_id = ?`, [e])))
        }

...
module.exports = function (app) {
    ...
    app.post('/khuyenmai', async (req, res) => {
        ...
        if (data.km_idsp) {
            ...
            if (_khuyenmai.length !== 0) return res.status(500).send("Sản phẩm đã tồn tại");
            delete data.km_idtl;
            ...
            let _values = [];
            await Promise.all(await _idsps.map(async e => await query(db, "DELETE FROM khuyen_mai WHERE km_idsp = ?", e.sp_id)))
            if (_idsps.length === 0) return res.status(500).send("Thể loại chưa có sản phẩm");
            _idsps.map(e => _values.push([e.sp_id, data.km_phantramgiam, data.km_ngaybatdau, data.km_ngayketthuc]));
            await query(db, "INSERT INTO khuyen_mai(km_idsp, km_phantramgiam, km_ngaybatdau, km_ngayketthuc) values ?", [_values]);
    ...
    app.put('/khuyenmai/:id', async (req, res) => {
        ...
        if (data.km_idsp) {
            ...
            if (_khuyenmai.length === 0) return res.status(500).send("Khuyến mãi tồn tại");
            delete data.km_idtl;
